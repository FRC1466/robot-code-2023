...
public class DriveSubsystem extends SubsystemBase {
  ...
  private void initializeTelemetry() {
    ...
    ShuffleboardLayout gyroLayout = teleTab.getLayout("gyro", BuiltInLayouts.kList).withSize(1, 3);
    gyroLayout.add("reset gyro position", new InstantCommand(() -> gyro.reset()));
    gyroRotEntry =
        gyroLayout.add("gyro rotation deg", gyro.getRotation2d().getDegrees()).getEntry();
    ...
    ShuffleboardLayout odometryLayout =
        teleTab.getLayout("odometry", BuiltInLayouts.kList).withSize(1, 3);
    ...
    for (SwerveModule module : swerveModules) {
      ShuffleboardLayout encoderLayout =
          teleTab
              ...
              .withSize(1, 3);
      cancoderEntries[module.moduleNumber] =
          encoderLayout.add("Cancoder", module.getCancoderAngle().getDegrees()).getEntry();
      integratedEntries[module.moduleNumber] =
          encoderLayout.add("Integrated", module.getPosition().angle.getDegrees()).getEntry();
      velEntries[module.moduleNumber] =
          encoderLayout.add("Velocity", module.getState().speedMetersPerSecond).getEntry();
    }
  }
  ...
  public void updateModuleTelemetry() {
    for (var module : swerveModules) {
      ...
      // SmartDashboard.putNumber("module " + module.moduleNumber + "cancoder deg",
      // module.getCancoderAngle().getDegrees());
  ...
  public void setSpeeds(double rad, double vx, double vy, boolean isFieldRelative) {
    var updatedRad =
        rad == 0 ? rad - headingController.calculate(Math.toRadians(gyro.getRate()), 0) : rad;
    if (isFieldRelative) {
  ...
  public void setSpeedsFieldRelativeAlternate(double rad, double vx, double vy) {
    ...
    var adjSpeeds =
        new ChassisSpeeds(twist_vel.dx / 0.02, twist_vel.dy / 0.02, twist_vel.dtheta / 0.02);
    speeds =
        ChassisSpeeds.fromFieldRelativeSpeeds(
            adjSpeeds.vxMetersPerSecond,
            adjSpeeds.vyMetersPerSecond,
            adjSpeeds.omegaRadiansPerSecond,
            getGyroRotation2d());
  }
