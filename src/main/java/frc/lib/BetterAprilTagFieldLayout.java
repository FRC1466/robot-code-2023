...
public class BetterAprilTagFieldLayout {
   ...
   */
  public BetterAprilTagFieldLayout(String path) throws IOException {
    this(Path.of(path));
  }
   ...
   * @throws IOException If reading from the file fails.
   */
  public BetterAprilTagFieldLayout(Path path) throws IOException {
    BetterAprilTagFieldLayout layout =
        new ObjectMapper().readValue(path.toFile(), BetterAprilTagFieldLayout.class);
   ...
   */
  public BetterAprilTagFieldLayout(List<AprilTag> apriltags, double fieldLength, double fieldWidth) {
    this(apriltags, new FieldDimensions(fieldLength, fieldWidth));
  }

  @JsonCreator
  private BetterAprilTagFieldLayout(
  ...
  @JsonIgnore
  public void flipLayoutByX(OriginPosition origin) {
    switch (origin) {
        case kBlueAllianceWallRightSide:
          setOrigin(new Pose3d());
          break;
   ...
   */
  public static BetterAprilTagFieldLayout loadFromResource(String resourcePath) throws IOException {
    try (InputStream stream = BetterAprilTagFieldLayout.class.getResourceAsStream(resourcePath);
        InputStreamReader reader = new InputStreamReader(stream)) {
      return new ObjectMapper().readerFor(BetterAprilTagFieldLayout.class).readValue(reader);
    }
  }

  @Override
  public boolean equals(Object obj) {
    if (obj instanceof BetterAprilTagFieldLayout) {
      var other = (BetterAprilTagFieldLayout) obj;
