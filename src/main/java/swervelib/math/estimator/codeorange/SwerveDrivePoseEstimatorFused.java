...
package swervelib.math.estimator.codeorange;
 ...
 *
 * <p>{@link SwerveDrivePoseEstimatorFused#update} should be called every robot loop. If your loops
 * are faster or slower than the default of 0.02s, then you should change the nominal delta time
 * using the secondary constructor: {@link
 * SwerveDrivePoseEstimatorFused#SwerveDrivePoseEstimator(Rotation3d, Pose3d, SwerveKinematics2,
 * Matrix, Matrix, Matrix, double)}.
 *
 * <p>{@link SwerveDrivePoseEstimatorFused#addVisionMeasurement} can be called as infrequently as
 ...
 */
public class SwerveDrivePoseEstimatorFused extends SwervePose {
   ...
   * @param accelStateStdDevs Standard deviations of the z axis robot speeds. Increase these numbers
   *     to trust the z axis speed state estimate less. In units of m/s.
   * @param accelMeasurementStdDevs Standard deviations of the z axis robot acceleration
   *     measurements. Increase these numbers to trust the z axis robot acceleration measurements
   *     less. In units of m/s/s.
   * @param visionMeasurementStdDevs Standard deviations of the vision measurements. Increase these
   *     numbers to trust global measurements from vision less. This matrix is in the form [x, y,
   *     theta]ᵀ, with units in meters and radians.
   */
  public SwerveDrivePoseEstimatorFused(
  ...
  public SwerveDrivePoseEstimatorFused(
    ...
    m_observer =
        new UnscentedKalmanFilter<>(
            ...
            stateStdDevs,
            localMeasurementStdDevs,
            AngleStatistics.angleMean(3),
            AngleStatistics.angleMean(0),
            AngleStatistics.angleResidual(3),
            AngleStatistics.angleResidual(0),
            AngleStatistics.angleAdd(3),
            m_nominalDt);
   ...
   * SwerveDrivePoseEstimatorFused#update} every loop.
   *
   * @param visionRobotPoseMeters The pose of the robot as measured by the vision camera.
   * @param timestampSeconds The timestamp of the vision measurement in seconds. Note that if you
   *     don't use your own time source by calling {@link
   *     SwerveDrivePoseEstimatorFused#updateWithTime} then you must use a timestamp with an epoch
   ...
   * SwerveDrivePoseEstimatorFused#setVisionMeasurementStdDevs(Matrix)} or this method.
   *
   * @param visionRobotPoseMeters The pose of the robot as measured by the vision camera.
   * @param timestampSeconds The timestamp of the vision measurement in seconds. Note that if you
   *     don't use your own time source by calling {@link
  ...
  public Pose3d update(
    ...
    return updateWithTime(
        WPIUtilJNI.now() * 1.0e-6, gyroAngle, accel, moduleStates, modulePositions);
  }
  ...
  public Pose3d updateWithTime(
    ...
    var u =
        VecBuilder.fill(
            ...
            omegaY,
            omegaZ);
    var newPose = updateThing(gyroAngle, modulePositions);
    // System.out.println(newPose.getZ());
    // System.out.println(fieldRelativeVelocities.toString());
    // System.out.println(modulePositions[0].toString());
    // System.out.println(getEstimatedPosition().getZ());
   ...
   */
  private Pose3d updateThing(Rotation3d gyroAngle, SwerveModulePosition[] modulePositions) {
  ...
  /**
   * Convert a {@link Pose3d} to a vector of [x, y, z, pitch, roll, yaw], where pitch, roll, yaw are
   * in radians.
   *
