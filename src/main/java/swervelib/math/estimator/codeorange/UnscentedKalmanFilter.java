 ...
 *
 * <p>The Unscented Kalman filter is similar to the {@link edu.wpi.first.math.estimator.KalmanFilter
 * Kalman filter}, except that it propagates carefully chosen points called sigma points through the
 * non-linear model to obtain an estimate of the true covariance (as opposed to a linearized version
 * of it). This means that the UKF works with nonlinear systems.
 */
...
public class UnscentedKalmanFilter<States extends Num, Inputs extends Num, Outputs extends Num>
  ...
  private final BiFunction<Matrix<States, N1>, Matrix<States, N1>, Matrix<States, N1>>
      m_residualFuncX;
  private final BiFunction<Matrix<Outputs, N1>, Matrix<Outputs, N1>, Matrix<Outputs, N1>>
      m_residualFuncY;
